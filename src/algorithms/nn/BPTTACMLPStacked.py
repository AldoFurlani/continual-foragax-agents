# T-BPTT counterpart of RealTimeActorCriticMLPStacked.
#
# Same backbone as that agent -- the LRU/S5 stacked-residual encoder of Orvieto
# et al. (2023) with RTUs in place of the LRU -- but the recurrence is unrolled
# over a time axis (BPTTLRTUs / BPTTNonLRTUs) instead of stepped by the
# real-time RTRL cell.
#
...
#   heads(LayerNorm(x))
#
# Two differences from RealTimeActorCriticMLPStacked, and only two:
#
#   1. Residual topology.  That agent uses the transformer convention of two
#      pre-norm residuals per block (one around the RTU sublayer, one around the
#      MLP).  LRU's SequenceLayer instead wraps a single skip around the whole
#      [recurrence -> MLP] block, which is what this reproduces.  The separate
#      rtu_proj is gone with it: the MLP is now the only route from the RTU's
#      2*d_hidden output back to the width-W stream.
#
#   2. Gradient scheme.  RealTimeLinearRTUs stop-gradients h_{t-1} and repairs
#      it with an exact per-layer RTRL trace, but that trace covers only the
#      cell's own parameters.  Stacking such cells therefore drops
#      d(h^(l+1)_t)/d(theta^(l)) -- the lower block never receives credit for
#      how its output persists in the block above.  Unrolling with nn.scan keeps
#      every time-by-depth path inside the truncation window instead.
#
...
class BPTTActorCriticMLPStacked(nn.Module):
    action_dim: int
    ...
    use_layernorm: bool = True  # interface parity; the stream is always normed
    mlp_expansion: int = 4
